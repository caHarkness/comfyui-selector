...
def parse_input(input_string, all_options={}):
    ...
    # Force prepend user's message, has ability to supress user's supplied options:
    if "message_prepend" in output:
        message_prepend = output["message_prepend"]
        using           = f"{message_prepend} {using}"
        using           = re.sub(r"\s+", " ", using)
        using           = using.strip()

    # Get batch size from long string:
    batch_re = r" x([1-9][0-9]{0,}?)$"
    match = re.search(batch_re, using)
    if match is not None:
        output["repeat_n_times"] = int(match.group(1))
    ...
    # Force prepend options:
    if "options_prepend" in output:
        options_prepend = output["options_prepend"]
        options = f"{options_prepend} {options}"
